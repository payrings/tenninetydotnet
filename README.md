...
Every safety property of the pipeline is mechanically enforced rather than requested: fail-closed networking, external host-owned gate state, disposable per-project test snapshots, locked dependency manifests, exact manifest scope, and semantic signature-drift hooks operate below the agents' permission level, so a misbehaving model can fail a gate but cannot move it.
...
| [`starter-kit/`](starter-kit/) | The pre-tested, executable template: the `dev.sh` orchestrator and all governance scripts, both Dockerfiles, the aider profile templates (`aider-conf/`), build/package manifests, pre-commit hooks, and annotated specification templates. Both guides copy from here – nothing is written by hand. You can also click **Use this template** on GitHub to start from a compliant repository. |
...
**To set up a new machine**: Work through [`SETUP_GUIDE.md`](SETUP_GUIDE.md) top to bottom until its verification checklist passes. You never repeat it per project.

**To start a project on a configured machine**: Go straight to [`WORKING_GUIDE.md`](WORKING_GUIDE.md), Phase 9. If you already have the full specification package in hand, [`docs/developer-quickstart.md`](docs/developer-quickstart.md) is the fastest route through both guides.

**To evaluate the framework before committing**: Read [`docs/1090-framework-explainer.md`](docs/1090-framework-explainer.md), and [`docs/scaling-to-medium-projects.md`](docs/scaling-to-medium-projects.md) if your target is larger than a small single-purpose tool.
