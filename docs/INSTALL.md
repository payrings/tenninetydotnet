...
This guide starts with a quick offline rehearsal and then builds the live environment: local GPU
inference on a single AMD Radeon RX 7900 XTX — by default through one llama-swap container
(Outcome B) — and, for the strongest isolation, the full KVM path where `tenninety`, aider,
generated code, builds, and tests are confined to a disposable KVM virtual machine (Outcome C).
...
| B. Container model service (default live path) | 1-4 and 19 | One llama-swap container serves two GGUF models from a single AMD GPU; live execution uses the Docker sandbox. |
| C. Isolated live execution | 5-17 | Agents run as a non-sudo user in a disposable KVM guest with no host mounts or device passthrough; llama-swap stays on the physical host behind an SSH tunnel. |
| D. Direct-host live mode (not isolated) | 18 | Clearly non-isolated fallback; never a default. |
...
| llama.cpp with working Vulkan backend | local inference (outcomes C/D) | physical host only |
| llama-swap | one-GPU model swapping (outcomes C/D) | physical host only |
| Two genuinely different GGUF models | coder and independent reviewer | outcome B: `models/`; outcomes C/D: physical host |
| aider | live coding-agent process | outcome B: coder image; VM only in the isolated path |
| Frontier-compatible HTTPS API and key | live planning and repair advice | key entered only in the live session |
| KVM/QEMU, libvirt, virt-manager | live isolation (outcome C) | physical host only |
| About 40 GB free for the two example GGUF files | local model storage | physical host only |
| About 50 GB sparse VM disk | disposable guest (outcome C) | physical host storage |
...
`spec.md` file is transferred into the guest, after the secure guest is ready (Section 14).
...
Use host port 8080 instead of the VM tunnel. Host port 8080 is also bound by the default llama-swap compose container (Section 19), so
