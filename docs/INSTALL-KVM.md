# Installing 10/90 with an isolated KVM agent on CachyOS

This guide builds the maximally isolated live environment: local GPU inference on the physical
CachyOS host, with `tenninety`, aider, generated code, builds, and tests confined to a
disposable KVM virtual machine. It is Outcome C of the [installation guide](INSTALL.md) —
complete its outcomes A (offline rehearsal) and B (container model service) there first. This
guide replaces the container model service with host-side llama-swap behind an SSH reverse
tunnel, because the guest never receives the GPU, the model files, or any host mount.
...
| --- | --- | --- |
| C. Isolated live execution | 1-14 | Agents run as a non-sudo user in a disposable KVM guest with no host mounts or device passthrough; the model service stays on the physical host. |
| D. Direct-host live mode (not isolated) | 15 | A clearly non-isolated fallback; never a default. |

Outcome C is the recommended configuration when agents work on anything sensitive: it is the
only path that adds a second hardware-backed boundary around the orchestrator.
...
## 2. Install local inference on the physical host

All commands in Sections 2 and 3 run on the **physical host**, not inside the VM.
...
## 3. Configure host-only llama-swap
...
## 4. Install KVM and libvirt on the physical host
...
## 5. Create and provision the base VM
...
## 6. Create the dedicated agent network on the physical host
...
## 7. Move the VM onto the secure network
...
## 9. Create the narrow llama-swap tunnel
...
## 10. Verify isolation before giving an agent work
...
## 11. Initialize IncidentDesk inside the VM
...
## 12. Configure isolated live mode
...
## 14. Export results and reset the VM
...
8. Before the next experiment, repeat the safe start order and all Section 10 isolation checks.
...
## 15. Direct-host live mode (not isolated)
...
llama-swap compose container (see the model-serving section of [INSTALL.md](INSTALL.md)), so
stop that stack first (`docker compose down`) or move one of the two listeners to a different
port before using this configuration:
...
- [Installation guide (outcomes A and B)](INSTALL.md)
